...
def add_leaflet_in_view() -> str:
    ...
    function calculatePctNoncriminal(totalCriminal, totalNoncriminal) {
        const total = totalCriminal + totalNoncriminal;
        if (total == 0) {
            return "N/A";
        }
        const pctNoncriminal = Math.round(100 * (totalNoncriminal / total));
    ...
    function calculateInViewStats(e) {
        ...
        const pctNoncriminal = calculatePctNoncriminal(totalCriminal, totalNoncriminal);
    ...
    ["zoomend", "moveend"].forEach(event => map.on(event, calculateInViewStats));
    """
    return html
